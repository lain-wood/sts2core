...
3.5 攻击方缩小 ×0.7
4.  攻击方虚弱 ×0.75
5.  防御方易伤 ×1.5（同一段累乘里还有巨像 / 翱翔 / 扑翼 / 包围 / 钢笔尖 / **钻石头冠 ×0.5**）
...
| `TurnStart` | **我的**回合开始，能量回满后、抽牌前 | 28 条。恶魔形态 绯红披风 滚石 盾墙 覆甲(玩家掉层，**第 2 回合起**) 巨像 好勇斗狠 抱抱先生 水银沙漏 灯笼 锚 弹珠袋 红面具 准备背包 烛台 开心小花 花粉核心 佩尔之肉 光耀 赤牛 摆动球 舵盘 **号角靴钉 宝石面具 碎石者 小血瓶 缩放仪 古茶具** **小提琴**(开局发牌 +2) |
...
| `TurnEnd` | **我的**回合结束，弃手牌前、敌人行动前 | 招架盾 **尖叫酒壶** **斗篷扣**(每张手牌 1 格挡) 覆甲(玩家给格挡) 奥利哈钢 再生 惊逃 缠绕 娇弱 历石 轰鸣 **波纹水盆**(本回合没打攻击) **钻石头冠**(本回合出牌 ≤ 2 ⇒ 挂减半) |
...
| `EnemyTurnEnd` | **敌人**整边行动完之后 | 高压 仪式 领地意识 逃跑大师 天敌 **熟睡**(熟睡甲虫：减层，归零醒来移除覆甲) **沉睡**(族母：减层；覆甲上一档已经摘了) **钻石头冠**(摘掉减半) |
| `PlayerSkill` | 打出技能牌，**牌结算之前** | 激怒（敌人持有）开信刀 活力火花 |
| `PlayerAttack` | 打出攻击牌，**牌结算之后** | 狂怒 精致折扇 苦无 杂耍 **手里剑 锁镰 风的女儿 双截棍** |
| `CardPlayed` | 打出任意牌 | 凋萎存在 娇弱 **流电**(电球头：打出能力牌挨 6 点，`TCond::LastPlayedKindIs`) **铁棒 棋子** |
...
> **「发牌」和「抽牌」是两条路**（2026-09-25，为小提琴分开的）。[源码] 只有 `CombatManager`
> 发开局手牌那一次是 `fromHandDraw: true`；`ModifyHandDraw` 那一族遗物（佩尔之血 / 准备背包 /
> 花粉核心 / 小提琴）改的是**那一次的张数**，别的一切（牌面抽牌、摆动球、百年积木、棋子…）都是真抽牌。
> 内核：`State::hand_draw_n`（只有 `open_hand` 调）vs `State::draw_n`（其余全部）。
> **`TOp::OwnerHandDraw` 不抽牌，只记账**（加进 `St::HandDrawBonus`，2026-09-25 起）：
> 发几张只在 `step::hand_draw_count` 一处算（基础 5 + 加张数 − 心灵腐化，封手牌上限），
> `open_hand` 按它发、planner 的机会节点按它枚举。原来在 `TurnStart` 上先抽，对 L1 逐张相同，
> 但那几张抽在机会节点之前。
> 平时两条路逐张相同，**只有小提琴和战斗专注的锁分得开它们**：我的回合里拦真抽牌、不拦发牌、
> 不在我的回合（`State::enemy_side`）也不拦。战斗专注的「不在我的回合就不拦」是同一天对齐的
> （[源码] 它在我的回合末就移除，内核的 `TURN_SCOPED` 要清到下一个回合开始）。
...
**`fire()` 按钩子分组扫**（2026-09-25）：`content::powers_for(hook)` 是 `POWERS` 按钩子过滤、
**组内保持表内顺序**的下标（第一次调用时建，`OnceLock`），`fire_ctx` 只扫那一组；只看钩子的五个开关
（`only_ctx` / `except_ctx` / `player_fires` / `enemies_fire` / `allow_dead`）提到循环外面算一次。
原来是每次点火整表线性扫 116 条、逐条比 `def.hook`。换完之后 `bin/bench` 单线程 **2.31 -> 3.58 M steps/s**
（+55%），语料九条验收逐字不变 —— 读数在 verification-log 09-25。
等价性押在 `powers_by_hook_is_the_table_filtered_in_order` 上：组内顺序一换，别的测试一条都不红
（今天没有一条规则的行为依赖同钩子上另一条的先后，这正是奥利哈钢那条注释要的），只有它红。
