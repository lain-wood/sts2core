...
### 对拍（2026-09-03，63 条实录 + 2 条合成 + 1 条被动）
...
| 整回合预测 | **2 帧不一致** | |
| 敌人出招预测 | 262/319 = **82%** 完全一致 | 允许集合平均 **1.10** 手/次（集合越大这个百分比越不值钱）；**1 只对不齐/未知**（**棘刺蟾蜍还没进 `ENEMIES`**，新语料带出来的）；**6 例落在允许集合外**（4 例是击晕那个已知洞，2 例还没归因）|
| L2 自洽性 | 220 回合，**0 例实战线赢过穷尽搜索** | 8 个回合没搜完（预算 200000），那几行是启发式排名不是解 |
| 跨回合 rollout（solver 策略） | P1 155/160 · P3(a)(b) 各 0 · P4 0/16192 | 硬判据全过 |
| 跨回合 rollout（fast 策略） | **✗ P4 红** | 生产口径（30 回合上限）有 **1/16192** 条推演没打完就被截断。**已归因**：是第 1 幕瀑布巨兽（240 血）在故意做弱的 `fast` 策略下的长尾 —— 中位 17 回合、尾巴到 41~48。抬高上限数字会变（32 回合就归 0）⇒ **不是挂死**。它只污染两条 opt-in 路径的期望（`--score mcts` / `--leaf rollout:`），**默认驾驶路径碰不到** |
| 跨回合 planner（D=2） | P5 配对 16192 对，均值 **+3.7 血**，更好 6301 / 更差 971 / 打平 8920 | 硬判据全过 |
| 同局面换随机种子，D=2 换不换线 | **225/294 = 77% 不变** | 也就是 **23% 的回合它换个种子就改主意**（单回合求解器是 0，回合内穷尽、确定）|
...
* **跨回合 planner（D=2）不当驾驶员**：它平均更好（+3.7 血），但**同一个局面换个
  随机种子有 23% 会改主意**。实战一个回合只发生一次，要的是单次可重复性。
  它在实战里只作为**分歧样本**并排报出来。
