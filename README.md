...
`State` 是 **POD + `Copy`**（3736 字节），快照和回滚就是一条 `memcpy`。
...
粗线条（2026-09-19）：79 条实录语料；第 1、2 幕的遭遇内核全开得出，第 3 幕差女王；
...
* **L3 的绝对水平不可信，只有成对的差值可信。** 回溯检验量到推演每场比玩家多掉约
  6.7 血；推演一瓶药水都不喝；引擎牌（每回合给格挡的 / 触发式的）在跨回合搜索里评不到，
...
CI（`.github/workflows/ci.yml`）每次推 `main` 都跑一遍。
...
data/       读源码得来的权威表（遭遇分布 · 敌人连接键 · 进阶数值）
